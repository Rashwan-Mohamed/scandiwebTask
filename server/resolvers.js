...
const resolvers = {
  Query: {
    ...
    getProductsByCategory: (_, { input }) =>
      data.categories.find((cat) => cat.name === input.title),
    getAllCategories: () => data.categories,
    getAllProducts: (_, { category }) => {
      let returned = [];
      if (category === "all") {
        return data.products;
      } else {
        data.products.map((prodict) => {
          if (prodict.category == category) {
            returned.push(prodict);
          }
        });
      }
      return returned;
    },
    GetCurrency: () => data.products[0].prices.currency,
  },
};
